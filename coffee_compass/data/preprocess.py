...
class CoffeePreprocessor:
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if (
            "altitude_mean_meters" in df.columns
            and df["altitude_mean_meters"].isna().any()
        ):

            # Remove unrealistic altitudes (coffee typically grows 500-3000m)
            invalid_altitude = (df["altitude_mean_meters"] > 3000) | (
                df["altitude_mean_meters"] < 0
            )
            if invalid_altitude.any():
                n_removed = invalid_altitude.sum()
                print(
                    f"   Removing {n_removed} rows with invalid altitude (>3000m or <0)"
                )
                df = df[~invalid_altitude].copy()

            # Fill with country-specific median (only for countries with data)
            country_medians = df.groupby("Country.of.Origin")[
                "altitude_mean_meters"
            ].median()
            df["altitude_mean_meters"] = df.apply(
                lambda row: (
                    country_medians.get(row["Country.of.Origin"], np.nan)
                    if pd.isna(row["altitude_mean_meters"])
                    else row["altitude_mean_meters"]
                ),
                axis=1,
            )
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["altitude_score"] = (
            df["altitude_mean_meters"] / 2000
        )  # Normalize around typical specialty coffee altitude

        # 3. Processing complexity score
        # Natural processing is more complex/risky but can yield unique flavors
        process_complexity = {"Natural": 3, "Honey": 2, "Washed": 1}
        df["processing_complexity"] = (
            df["Processing.Method"].map(process_complexity).fillna(1)
        )
    ...
    def preprocess_pipeline(
        self, filepath: str, is_training: bool = True, verbose: bool = False
    ) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        if verbose:
            ...
            axes.plot(
                X["Country.of.Origin_Honduras"], "b*", linewidth=2, label="Analytical"
            )
            plt.tight_layout()
            plt.show()
